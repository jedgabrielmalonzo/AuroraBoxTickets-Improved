...
function fetchParksBySubcategory($conn, $subcategoryName, $limit = 5) {
    ...
    $cat_sql = "SELECT category_id FROM category WHERE category_name = 'Museums'";
...
function getMuseumSubcategories($conn) {
    $cat_sql = "SELECT category_id FROM category WHERE category_name = 'Museums'";
    $cat_result = $conn->query($cat_sql);
...
$subcategories = getMuseumSubcategories($conn);
...
<div class="page-header">
    <div class="container">
        <h1>Museums</h1>
        <p>Discover fascinating museums filled with history, art, and unforgettable stories</p>
    </div>
</div>
...
foreach ($allParks as $park) {
    ...
    if (!empty($park['created_at'])) {
        try {
            ...
            if ($diffSec >= 0 && $diffSec <= 7 * 86400) { // 14 days
                $newParks[] = $park;
            }
        } catch (Exception $e) {
            // skip invalid date
...
<div class="PARKSHOW">
    ...
    <?php if (!empty($newParks)): ?>
        <section id="newly-added">
            <h2>NEWLY ADDED</h2>
            <p class="section-note">These museums were added in the past 2 weeks</p>
                    ...
                    ?>
                        <a href="park_info.php?id=<?= $parkId ?>" class="card">
                            ...
                            <div class="card-content">
                                <div class="category"><?= htmlspecialchars($park['subcategory_name'] ?? 'Museum') ?> • <?= htmlspecialchars($park['city'] ?? 'Philippines') ?></div>
...
<?php if (empty($subcategories) && empty($newParks)): ?>
    <div class="no-data">
        <h3>No museums available at the moment.</h3>
    </div>
<?php endif; ?>
...
<script>
   ...
   // ✅ Wishlist / Cart / Account custom handling
