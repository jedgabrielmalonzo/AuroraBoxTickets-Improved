...
function fetchParksBySubcategory($conn, $subcategoryName, $limit = 5) {
    // First get Nature Parks category ID
    $cat_sql = "SELECT category_id FROM category WHERE category_name = 'Nature Parks'";
    $cat_result = $conn->query($cat_sql);
    
    if ($cat_result->num_rows > 0) {
        $category = $cat_result->fetch_assoc();
        $nature_parks_id = $category['category_id'];
        ...
        $subcat_stmt->bind_param("si", $subcategoryName, $nature_parks_id);
        $subcat_stmt->execute();
...
// Fetch all nature park subcategories
function getNatureParkSubcategories($conn) {
    $cat_sql = "SELECT category_id FROM category WHERE category_name = 'Nature Parks'";
    $cat_result = $conn->query($cat_sql);
    ...
    if ($cat_result->num_rows > 0) {
        ...
        $subcat_stmt = $conn->prepare($subcat_sql);
        $subcat_stmt->bind_param("i", $nature_parks_id);
        $subcat_stmt->execute();
        $subcat_result = $subcat_stmt->get_result();
...
// Get all nature park subcategories
$subcategories = getNatureParkSubcategories($conn);
...
<div class="page-header">
    <div class="container">
        <h1>Nature Parks</h1>
        <p>Discover breathtaking nature parks with scenic trails and peaceful escapes</p>
    </div>
</div>
...
<div class="PARKSHOW">
   ...
   <?php if (!empty($newParks)): ?>
    <section id="newly-added">
        <h2>NEWLY ADDED</h2>
        <p class="section-note">These nature parks were added in the past 2 weeks</p>
                ...
                ?>
                    <a href="park_info.php?id=<?= $parkId ?>" class="card">
                        ...
                        <div class="card-content">
                            <div class="category"><?= htmlspecialchars($park['subcategory_name'] ?? 'Nature Park') ?> • <?= htmlspecialchars($park['city'] ?? 'Philippines') ?></div>
...
<?php endif; ?>
   ...
   <!-- SUBCATEGORY SECTIONS (lahat ng parks, kasama rin yung NEW) -->
...
<?php if (empty($subcategories) && empty($newParks)): ?>
    <div class="no-data">
        <h3>No nature parks available at the moment.</h3>
    </div>
<?php endif; ?>
